 ...
 * @param {string} options.message.payload.playerDetails.Top.serverId
 ...
 * @param {string} options.message.payload.playerDetails.Mid.serverId
 ...
 * @param {string} options.message.payload.playerDetails.Jg.serverId
 ...
 * @param {string} options.message.payload.playerDetails.Bot.serverId
 ...
 * @param {string} options.message.payload.playerDetails.Supp.serverId
 * @param {string} options.message.payload.serverId - The Discord server that the Team belongs to.
...
service.publishTeamEventBasedOnServer = async (ws, { message, path, query }) => {
  ...
  if (wsClients.length > 0) {
    ...
    wsClients.forEach(client => {
      if (client !== ws
        && client.readyState === 1
        && client.server === parsedMessage.payload.serverId) {
