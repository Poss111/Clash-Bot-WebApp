...
export class TeamsDashboardComponent implements OnInit, OnDestroy {
    ...
    private syncTeamInformation(data: ClashTeam[], userDetails: UserDetails) {
        if (data.length < 1) {
            this.teams = [{error: 'No data'}];
        } else {
            ...
            this.applicationDetailsService.getApplicationDetails()
                ...
                .subscribe((applicationDetails) => {
                    console.log('Updating eligible tournaments...');
                    if (applicationDetails && applicationDetails.currentTournaments) {
                        ...
                        console.log(`New list ${JSON.stringify(this.eligibleTournaments)}`);
                    }
                });
        }
    }
    ...
    createNewTeam(element: MatOption) {
        element.select();
        console.log(this.createNewTeamFormControl.value);
